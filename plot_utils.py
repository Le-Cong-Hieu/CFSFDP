...
def plot_scatter_diagram(which_fig, x, y, x_label = 'x', y_label = 'y', title = 'title', style_list = None):
	...
	'''
	styles = ['k', 'g', 'r', 'c', 'm', 'y', 'b', '#9400D3','#C0FF3E','k', 'g', 'r', 'c', 'm', 'y', 'b', '#9400D3','#C0FF3E',
	   'k', 'g', 'r', 'c', 'm', 'y', 'b', '#9400D3','#C0FF3E','k', 'g', 'r', 'c', 'm', 'y', 'b', '#9400D3','#C0FF3E',]
	assert len(x) == len(y)
	# if style_list is not None:
	# 	#assert len(x) == len(style_list) and len(styles) >= len(set(style_list)), "Lengths of x and style_list must match, and there must be enough unique values in style_list"
